...
const DrawerList = [
  ...
  { icon: 'invoice_pos', label: 'Đơn delivery', navigateTo: NAVIGATION_DELIVERY_ORDER },
...
const DrawerContent = ({ onPressShiftClose, ...props }) => {
  ...
  useEffect(() => {
    if ((currentScreen === NAVIGATION_APP_ORDER || currentScreen === NAVIGATION_DELIVERY_ORDER) && onlineOrders && onlineOrders.length > 0) {
